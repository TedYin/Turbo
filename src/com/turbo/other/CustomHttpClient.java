...
public class CustomHttpClient {
	...
	public static String PostFromWebByHttpClient(Context context, String url,
		...
		try {
			...
			throw new RuntimeException(TurboException.DefaultErrorType.CONNECTION_TIME_OUT_EXCEPTION);
		} 
	}

	public static String getFromWebByHttpClient(Context context, String url,
			NameValuePair... nameValuePairs) throws Exception{
			TurboLog.d("getFromWebByHttpClient url = " + url);
		try {
			// http地址
			// String httpUrl =
			...
			if (httpResponse.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
				throw new RuntimeException(TurboException.DefaultErrorType.NET_ERROR_EXCEPTION);
			}
			return EntityUtils.toString(httpResponse.getEntity());
		} catch (ParseException e) {
			throw new RuntimeException(new TurboException("数据解析异常！"));
		} catch (IOException e) {
			e.printStackTrace();
			throw new RuntimeException(TurboException.DefaultErrorType.NO_RESPONSE_EXCEPTION);
		} 	
	}
	...
	private static synchronized HttpClient getHttpClient(Context context) {
		if (null == customerHttpClient) {
			...
			if (EnvHelper.getNetworkType(context) == EnvHelper.NetType.WIFI) {
